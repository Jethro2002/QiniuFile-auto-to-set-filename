 ...
 * @author LiCxi & Jieck
 * @version 1.0.0
 * @link https://github.com/Jieck2002/QiniuFile-auto-to-set-filename
 * @date 2018-9-22
 */
...
class QiniuFile_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function uploadFile($file, $content = null)
    {
        ...
        //设置图片名称格式，Y年,m月,d天,h时,i分,s秒，$part[1]为文件后缀名
        $upload_filename = date("Ymdhis") . '.' . $part[1];
        list($ret, $error) = $upManager->putFile($token, $option->savepath . $upload_filename, $filename);
        ...
        if ($error == null)
        {
            ...
            (
                ...
                'path'  =>  $option->savepath . $upload_filename.( $option->imgstyle == '' ? '' : '-'.$option->imgstyle ),
                'size'  =>  $file['size'],
                'type'  =>  $ext,
